    ...
    return this.filter("img")
      .each(function () {
        ...
        $srcImg.one("load", function () {
          ...
          var coveredWidth =
            cols * (2 * computedStyles.piece.border) +
            (cols - 1) * style.gridSize;
          var coveredHeight =
            rows * (2 * computedStyles.piece.border) +
            (rows - 1) * style.gridSize;
          ...
          var totalPieceWidth =
            width + 2 * computedStyles.piece.border + style.gridSize;
          var totalPieceHeight =
            height + 2 * computedStyles.piece.border + style.gridSize;
          ...
          var boxModelHack = {
            ...
            wrapper: $.boxModel
              ? 0
              : 2 *
                (computedStyles.wrapper.border +
                  computedStyles.wrapper.padding),
            gui: {
              width: $.boxModel
                ? 0
                : 2 * computedStyles.gui.border +
                  computedStyles.gui.padding.left +
                  computedStyles.gui.padding.right,
              height: $.boxModel
                ? 0
                : 2 * computedStyles.gui.border +
                  computedStyles.gui.padding.top +
                  computedStyles.gui.padding.bottom,
            },
          };
          ...
          var shuffle = function (rounds, speed) {
            ...
            if (speed) {
              ...
              if (
                control.confirmShuffle &&
                moves > 0 &&
                !window.confirm(texts.confirmShuffleMessage)
              )
                return false;
                ...
                if (speed === undefined) {
                  ...
                  $piece.animate(
                    ...
                    function () {
                      ...
                      if (
                        animCounter ==
                        animation.shuffleRounds * (rows * cols - 1)
                      ) {
                        lock = false;
                        animCounter = 0;
                      }
          ...
          for (var i = 0; i < rows; i++) {
            for (var j = 0; j < cols; j++) {
              ...
              var bgLeft =
                -1 * (j * totalPieceWidth + computedStyles.piece.border);
              var bgTop =
                -1 * (i * totalPieceHeight + computedStyles.piece.border);
            ...
            .css({
              width:
                ...
                2 *
                  (computedStyles.wrapper.padding +
                    computedStyles.wrapper.border) +
                boxModelHack.gui.width,
              height:
                fullHeight +
                2 *
                  (computedStyles.wrapper.padding +
                    computedStyles.wrapper.border) +
                boxModelHack.gui.height,
              textAlign: "left",
              overflow: "hidden",
          ...
          $pieces.click(function () {
            ...
            if (
              Math.abs(source.row - dest.row) +
                Math.abs(source.col - dest.col) !=
              1
            ) {
              lock = false;
              return false;
          ...
          if (control.toggleOriginal)
            $originalButton.click(function () {
              ...
              if ($originalButton.is(".jqp-toggle")) {
                ...
                if (control.shufflePieces)
                  $shuffleButton.removeClass("jqp-disabled");
                if (control.toggleNumbers)
                  $numbersButton.removeClass("jqp-disabled");
                ...
                if (control.shufflePieces)
                  $shuffleButton.addClass("jqp-disabled");
                if (control.toggleNumbers)
                  $numbersButton.addClass("jqp-disabled");
                ...
                $background
                  .appendTo($wrapper)
                  .fadeTo(animation.fadeOriginalSpeed, 1, function () {
                    lock = false;
                  });
              }
          ...
          var finishGame = function () {
            if (success.fadeOriginal) {
              // disable buttons
              if (control.toggleOriginal)
                $originalButton.addClass("jqp-disabled");
              if (control.toggleNumbers)
                $numbersButton.addClass("jqp-disabled");
              ...
              $background
                .appendTo($wrapper)
                .fadeTo(animation.fadeOriginalSpeed, 1.0, function () {
